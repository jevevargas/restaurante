...
//  $orden=$_REQUEST['orden']; // echo $orden;
    ...
    INNER JOIN bodega ON componentes.id_bodega=bodega.id_bodega WHERE  estadoplato='0' AND compocicion='1' GROUP BY nomplato LIMIT 6";

    if (isset($_POST['consulta'])) {
    	$q = $enlace->real_escape_string($_POST['consulta']);
    	$query = "SELECT nomplato,GROUP_CONCAT(DISTINCT producto_bodega), bodega.id_bodega, plato.precioplato, plato.estadoplato, plato.idplato,descplato
        FROM plato INNER JOIN componentes ON componentes.idplato=plato.idplato
               INNER JOIN bodega ON componentes.id_bodega=bodega.id_bodega WHERE estadoplato='0'  AND compocicion='1' 
               AND nomplato  LIKE '%$q%' ";
    }
    ...
    if ($resultado->num_rows>0) {
    	...
    	while ($fila = $resultado->fetch_assoc()) {
    		...
    		<td style='text-align:center; font-size:12px;'>
			<button class='btn btn-warning btn-sm btn-block ordenar' data-toggle='modal' data-target='#ordenar' value=" .$fila['idplato'].">".$fila['GROUP_CONCAT(DISTINCT producto_bodega)']."</button>
			</td>
            ...
            <td style='text-align:center' >
            <button class='btn btn-light btn-sm btn-block activar' data-toggle='modal' data-target='#activar' value=" .$fila['idplato']."><i class='bi bi-toggle-on' style='color:#B20000;font-size:25px;'></i></button>
