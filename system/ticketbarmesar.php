...
<br>
        ...
        $pedido = $pdo->prepare(" SELECT SUM(cantidadorden) AS cantidades, nomorden,idventa, precioventa , impresionplato,impresionplato, acocina
        FROM venta LEFT JOIN plato ON venta.idplato=plato.idplato
        WHERE  fechaorden  BETWEEN  '$inicio' AND '$final' 
        AND orden='$orden' AND estadoorden='0' AND impresionplato='2' AND acocina='1'  AND venta.impresion IN ('1','0')   GROUP BY plato.idplato ");
        $pedido->execute();
