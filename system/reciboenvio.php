<!DOCTYPE html>
 ...
 while ($resul = $estado->fetch()) {
     $idmesa=$resul -> idcliente;  //echo $idmesa;
 }
 $estado = $pdo->prepare(" SELECT * FROM cliente WHERE idcliente='$idmesa' AND estadocliente='1' ");
 $estado->execute();
 while ($resul = $estado->fetch()) {  
   $cliente= $resul ->nomcliente;
   $direccioncliente= $resul ->direccioncliente;
   $dui= $resul ->dui;
 }
?>
 <div class="fecha"><?php echo $dia=date('d-m-Y');  ?></div>
 <div class="nom"><?php echo $cliente;  ?></div>
 <div class="nom"><?php echo $direccioncliente;  ?></div>
