<div class="col-md-12">
    <div class="row">
        ...
        <?php
        if ($estadocaja == 1) {
            $todalazona = $pdo->prepare(" SELECT * FROM mesa left join cliente on mesa.idmesa=mesa.idmesa  WHERE mesa.estadomesa='1' AND estadocliente='1' ");
            $todalazona->execute();
            while ($resultb = $todalazona->fetch()) {
        ?>

                <div class="col-md-6 cajapedidos">
                    <a href="detallecaja?id=<?php echo $resultb->idmesa; ?>" class="btn btn-info btn-block rounded-0">
                        <div class="row">
                            <div class="col-md-12"><?php echo $resultb->nombremesa;  ?> (<?php echo $resultb->nomcliente;  ?>)</div>
                        </div>
                    </a>
